#!/usr/bin/env python3
"""Evaluate five bulb scales and store the first episode in one structured memmap."""
...
parser.add_argument("--cache", default=None, help="Optional five-scale grasp-cache prefix.")
...
def _reshape_cpu(tensor: torch.Tensor, num_envs_per_scale: int) -> np.ndarray:
    shape = (len(SCALES), num_envs_per_scale) + tuple(tensor.shape[1:])
...
def _record_state(data: np.memmap, raw_env, actions: torch.Tensor, active: torch.Tensor, step: int) -> None:
    ...
    active_np = _reshape_cpu(active, num_envs_per_scale).astype(bool)
    ...
    for name, tensor in state_fields.items():
        values = _reshape_cpu(tensor, num_envs_per_scale)
...
def main(env_cfg: DirectRLEnvCfg, agent_cfg: dict) -> None:
    ...
    env_cfg.scale_range = [SCALES[0], SCALES[-1], len(SCALES)]
    ...
    try:
        ...
        with torch.inference_mode():
            for step in range(args_cli.episode_steps):
                ...
                active_np = _reshape_cpu(active, args_cli.num_envs_per_scale).astype(bool)
                data["reward"][:, :, step][active_np] = _reshape_cpu(rewards, args_cli.num_envs_per_scale)[active_np]
                truncated = infos.get("time_outs", torch.zeros_like(dones, dtype=torch.bool)).bool()
                terminated = dones.bool() & ~truncated
                data["truncated"][:, :, step] = _reshape_cpu(truncated & active, args_cli.num_envs_per_scale)
                data["terminated"][:, :, step] = _reshape_cpu(terminated & active, args_cli.num_envs_per_scale)
                active &= ~dones.bool()
